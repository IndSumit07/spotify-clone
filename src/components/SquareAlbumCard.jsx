...
const SquareAlbumCard = ({props}) => {
  return (
    <Link to={`/album/${props.id}`} state={{title:props.title, coverImage:props.coverImage, singer:props.singer}} className='h-[190px]'>
      ...
      <div className='text-[12px] mt-1 ml-1 font-semibold '>
        {props.title}
      </div>
    </Link>
  );
